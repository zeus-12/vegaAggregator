...
class TableService extends BaseService {
  ...
  async fetchTablesByFilter(filter_key, unique_id) {
    ...
    switch (filter_key) {
      ...
      case "name": {
        path =
          '/accelerate_tables/_design/filter-tables/_view/filterbyname?startkey=["' +
          unique_id +
          '"]&endkey=["' +
          unique_id +
          '"]';
      ...
      case "section": {
        path =
          '/accelerate_tables/_design/filter-tables/_view/filterbysection?startkey=["' +
          unique_id +
          '"]&endkey=["' +
          unique_id +
          '"]';
      ...
      case "kot": {
        path =
          '/accelerate_tables/_design/filter-tables/_view/filterbyKOT?startkey=["' +
          unique_id +
          '"]&endkey=["' +
          unique_id +
          '"]';
      ...
      case "savedorders": {
        path = "/accelerate_tables/_design/filter-tables/_view/filterbysavedorders";
        break;
      }
      default: {
    ...
    if (filter_key == "all" || filter_key == "live" || filter_key == "section") {
      //Set of tables
  ...
  async addNewTableSection(sectionName) {
    return await this.SettingsService.addNewEntryToSettings("ACCELERATE_TABLE_SECTIONS", {
  ...
  async deleteTableSection(sectionName) {
    ...
    let isAllFree = true;
    for (var i = 0; i < tableData.length; i++) {
      //validate if all tables are free
    ...
    await this.SettingsService.removeEntryFromSettings("ACCELERATE_TABLE_SECTIONS", {
  ...
  async tableTransferKOT(kotId, newTableNumber) {
    ...
    if (kotData.table == newTableNumber) {
      //same table
  ...
  async mergeKOT(branch, finalTable, mergeTableList) {
    ...
    while (kotCounter < kotCount) {
      ...
      if (mergingBillingMode == "") {
        ...
        if (mergingBillingMode != kotfile.orderDetails.mode) {
          ...
          throw new ErrorResponse(
            ...
            "Operation Aborted! Orders have to be billed under same mode",
            "#e74c3c"
          );
        }
      }
      ...
      for (var g = 0; g < kotfile.extras.length; g++) {
        ...
        if (!extraDuplicateFlag) {
          //No duplicate, push the extra wholely.
      ...
      for (var f = 0; f < kotfile.cart.length; f++) {
        ...
        if (!itemDuplicateFlag) {
          //No duplicate, push the item wholely.
  ...
  async clearSavedOrderMappingFromTables() {
    //removes all the "Saved Mapping" from Tables.
    let data = {};
    let responseList = [];

    data = await this.fetchTablesByFilter("savedorders", "").catch((error) => {
      throw error;
    });
    ...
    if (tableData.length == 0) {
      throw new ErrorResponse(ResponseType.NO_RECORD_FOUND, ErrorType.no_table_mapped_to_this_session);
    }

    for (var i = 0; i < tableData.length; i++) {
      var remember_id = tableData[i].value._id;
      var remember_rev = tableData[i].value._rev;

      tableData[i].value.assigned = "";
      tableData[i].value.remarks = "";
      tableData[i].value.KOT = "";
      tableData[i].value.status = 0;
      tableData[i].value.lastUpdate = "";
      tableData[i].value.guestName = "";
      tableData[i].value.guestContact = "";
      tableData[index].value.reservationMapping = "";
      tableData[i].value.guestCount = "";

      data = await this.TableModel.updateTable(remember_id, tableData[i].value).catch((error) => {
        throw error;
      });
  ...
  async addTableToReserveList(tableID, optionalComments) {
    let data = {};
    data = await this.fetchTablesByFilter("name", tableID).catch((error) => {
      throw error;
    });
    ...
    if (data.rows.length != 1) {
      throw new ErrorResponse(ResponseType.NO_RECORD_FOUND, ErrorType.tables_data_not_found);
    }
    ...
    var timestamp = moment().format("HHmm");

    tableData.assigned = optionalComments;
    tableData.remarks = "";
    tableData.KOT = "";
    tableData.status = 5;
    ...
    return await this.TableModel.updateTable(remember_id, tableData).catch((error) => {
      throw error;
    });
  }

  async removeTableFromReserveList(tableID) {
    let data = {};
    data = await this.fetchTablesByFilter("name", tableID).catch((error) => {
      throw error;
    });
