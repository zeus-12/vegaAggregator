...
class LicenseService extends BaseService {
  ...
  async addNewLicense(newLicenseObject) {
    ...
    await this.SettingsService.addNewEntryToSettings("ACCELERATE_CONFIGURED_MACHINES", newLicenseObject).catch(error => {
      throw error
    });

    var newEntry = { 
      "systemName": machine, 
      "data": [ { "name": "theme", "value": "skin-green" }, 
    ...
    await this.SettingsService.addNewEntryToSettings("ACCELERATE_PERSONALISATIONS", newEntry).catch(error => {
      throw error
    });

    var newEntry = { 
      "systemName": machine, 
      "data": [{ "name": "notifications", "value": "ALL" }, 
    ...
    await this.SettingsService.addNewEntryToSettings("ACCELERATE_SYSTEM_OPTIONS", newEntry).catch(error => {
      throw error
    });

    var newEntry = { 
      "systemName": machine, 
      "data": [ { "name": "Show Spotlight Search", "value": "shift+f" }, 
    ...
    await this.SettingsService.addNewEntryToSettings("ACCELERATE_SHORTCUT_KEYS", newEntry).catch(error => {
      throw error
    });
    ...
    await this.SettingsService.addNewEntryToSettings("ACCELERATE_CONFIGURED_PRINTERS", newEntry).catch(error => {
      throw error
    });

    await this.SettingsService.addNewEntryToSettings("ACCELERATE_KOT_RELAYING", newEntry).catch(error => {
      throw error
    });
